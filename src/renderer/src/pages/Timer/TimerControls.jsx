...
export default function TimerControls({
  mode, isRunning,
  onStart, onPause, onSkip, onStop, onReset,
  ...
  function handleMinutesChange(val) {
    setInputMinutes(val)
    setActivePreset(null)
    onDurationChange(val * 60 + inputSeconds) // uses fresh val, stable inputSeconds
  }

  function handleSecondsChange(val) {
    setInputSeconds(val)
    setActivePreset(null)
    onDurationChange(inputMinutes * 60 + val) // uses fresh val, stable inputMinutes
  }

  const isDirty = isRunning
    ? false
    : (inputMinutes * 60 + inputSeconds) !== totalSeconds
  ...
  return (
    <Stack align="center" gap="md">
        ...
        <Stack align="center" gap="sm">
          <Group gap="xs">
            {PRESETS.map(p => (
              <ActionIcon
                key={p.label}
                variant={activePreset === p.label ? 'filled' : 'light'}
                color="pink" size="lg" radius="xl"
                onClick={() => handlePreset(p)}
              >
                <Text size="xs" fw={600}>{p.label}</Text>
      ...
      <Group justify="center" gap="xl">
        {isRunning ? (
          <>
            <ActionIcon variant="filled" color="pink" size="xl" radius="xl" onClick={onPause}>
              <Pause size={16} />
            </ActionIcon>
            {mode === 'focus' ? (
              ...
              <ActionIcon variant="filled" color="pink" size="xl" radius="xl" onClick={onStop}>
                <Square size={16} />
              </ActionIcon>
            )}
          </>
        ) : (
          <ActionIcon variant="filled" color="pink" size="xl" radius="xl" onClick={onStart}>
            <Play size={16} />
          </ActionIcon>
        )}
        {/* only show reset when there's something to reset */}
        {!isRunning && totalSeconds > 0 && (
          <ActionIcon variant="filled" color="pink" size="xl" radius="xl" onClick={onReset}>
            <RotateCcw size={16} />
          </ActionIcon>
        )}
      </Group>
